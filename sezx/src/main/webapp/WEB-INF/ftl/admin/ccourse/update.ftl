...
<div class="kit-doc">
    <form class="layui-form layui-form-pane" action="">
        ...
        <input type="hidden" id="id" name="id" value="${(ccourse.id)!}">
        <div class="layui-form-item">
            <label class="layui-form-label">课程名</label>
            <div class="layui-input-block">
                <input id="courseName" name="courseName" value="${ccourse.courseName!}" lay-verify="required"
                       placeholder="请输入课程名"
                       autocomplete="off"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="courseDescribe" value="${ccourse.courseDescribe!}" lay-verify="required"
                       placeholder="请输入课程描述" autocomplete="off"
                       class="layui-input"
                       type="text">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="courseName2" id="courseName2" value="${ccourse.courseName!}" lay-verify="required"
                       readonly placeholder="请上传视频"
                       autocomplete="off"
                       class="layui-input" type="text">
            </div>
        </div>
        <div class="layui-form-item">
            <video id="example_video" class="video-js vjs-default-skin vjs-big-play-centered" controls preload="none" width="640"
                ...
                <source id="mp4source" src="${ccourse.courseUrl!'http://视频地址格式1.mp4'}" type='video/mp4' />
                <track kind="captions" src="demo.captions.vtt" srclang="en" label="English"></track><!-- Tracks need an ending tag thanks to IE9 -->
                <track kind="subtitles" src="demo.captions.vtt" srclang="en" label="English"></track><!-- Tracks need an ending tag thanks to IE9 -->
            </video>
        </div>
        <input type="hidden" name="courseUrl" id="courseUrl" value="${ccourse.courseUrl!}">


        <div class="layui-form-item">
            <label class="layui-form-label">上架价格</label>
            <div class="layui-input-block">
                <input name="goodsPrice" value="${ccourse.goodsPrice}" lay-verify="required" readonly placeholder="请输入价格"
                       autocomplete="off"
                       class="layui-input"
                       type="text">
            </div>
...
<script>
    ...
    layui.use(['form', 'layedit','table','laydate','upload'], function(){
        ...
        form.on('submit(update)', function(data){
            $.post("${basePath}/ccourse/update",data.field,function(data){
                // 获取 session
                if(data.status!=200){
