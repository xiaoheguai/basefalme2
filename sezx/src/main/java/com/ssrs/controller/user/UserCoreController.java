...
public class UserCoreController extends BaseController {
    ...
    public ModelAndView index(HttpServletRequest request) {
        ...
        for(int i = menus.size()-1;i>=0;i--){
            ...
            if("订单明细".equals(menu.getTitle()) ||
                    "评论学员管理".equals(menu.getTitle()) ||
                    "评论管理课程".equals(menu.getTitle()) ||
                    "试题必修管理".equals(menu.getTitle()) ||
                    "前台页面导航栏配置".equals(menu.getTitle())
            ){
                menus.remove(menu);
                continue;
            }
            if(menu.getChildren()!=null && menu.getChildren().size()>0){
                for (int j=menu.getChildren().size()-1;j>=0;j--){
                    Menu me = menu.getChildren().get(j);
                    if(me.getTitle()==null || me.getTitle() == ""){
                        menu.getChildren().remove(menu);
                        continue;
                    }
                    if("订单明细".equals(me.getTitle()) ||
                            "评论学员管理".equals(me.getTitle()) ||
                            "评论管理课程".equals(me.getTitle()) ||
                            "试题必修管理".equals(me.getTitle()) ||
                            "前台页面导航栏配置".equals(me.getTitle())
                    ){
                        menu.getChildren().remove(me);
                    }
                }
            }
