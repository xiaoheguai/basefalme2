package com.ssrs.controller.reception.token.mamager;

import cn.hutool.crypto.digest.DigestUtil;
import com.ssrs.controller.reception.token.MSampleRealm;
import com.ssrs.controller.reception.token.MShiroToken;
import com.ssrs.core.shiro.session.CustomSessionManager;
import com.ssrs.core.shiro.token.SampleRealm;
import com.ssrs.core.shiro.token.ShiroToken;
...
/**
* @Description:    shiro 管理下的工具类
* @Author:         ssrs
* @CreateDate:     2018/8/6 14:59
* @UpdateUser:     ssrs
* @UpdateDate:     2018/8/6 14:59
* @Version:        1.0
*/
public class MTokenManager {
	//用户登录管理
	public static final MSampleRealm realm = SpringContextUtil.getBean("msampleRealm",MSampleRealm.class);
	//用户session管理
	 ...
	 * @return
	 */
	public static MUserVo getToken(){
		MUserVo token = (MUserVo) SecurityUtils.getSubject().getPrincipal();
		return token ;
	}
	 ...
	 * @return
	 */
	public static MUserVo login(MUserVo user,Boolean rememberMe){
		MShiroToken token = new MShiroToken(user.getLoginId(), DigestUtil.md5Hex(user.getLoginPwd()));
	 ...
	 * @return
	 */
	public static MUserVo relogin(MUserVo user,Boolean rememberMe){
		MShiroToken token = new MShiroToken(user.getLoginId(), user.getLoginPwd());
		token.setRememberMe(rememberMe==null?false:rememberMe);
		SecurityUtils.getSubject().login(token);
		return getToken();
	}
	...
	public static void clearNowUserAuth(){
		 ...
		 * 获取到的时候是一个集合。Collection<Realm> 
			RealmSecurityManager securityManager =
		  	...
		  	SampleRealm token = (SampleRealm)securityManager.getRealms().iterator().next();
		 */
		/**
